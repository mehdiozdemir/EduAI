...
export interface QuestionCountSelectorProps {
  value: QuestionCountOption | null;
  onChange: (value: QuestionCountOption) => void;
  error?: string;
  disabled?: boolean;
  className?: string;
  label?: string;
...
const QUESTION_COUNT_OPTIONS: Array<{
...
}> = [
  {
    value: 5,
    label: '5 Soru',
    description: 'Hızlı bir pratik için ideal',
    color: 'text-blue-600 border-blue-200 bg-blue-50 hover:bg-blue-100 focus:ring-blue-500'
  },
  {
    value: 10,
    label: '10 Soru',
    description: 'Orta uzunlukta bir quiz',
    color: 'text-green-600 border-green-200 bg-green-50 hover:bg-green-100 focus:ring-green-500'
  },
  {
    value: 15,
    label: '15 Soru',
    description: 'Kapsamlı bir değerlendirme',
    color: 'text-yellow-600 border-yellow-200 bg-yellow-50 hover:bg-yellow-100 focus:ring-yellow-500'
  },
  {
    value: 20,
    label: '20 Soru',
    description: 'Detaylı bir sınav deneyimi',
    color: 'text-purple-600 border-purple-200 bg-purple-50 hover:bg-purple-100 focus:ring-purple-500'
  }
];
...
const QuestionCountSelector: React.FC<QuestionCountSelectorProps> = ({
  ...
  className,
  label = 'Soru Sayısı',
  helperText = 'Quiz için soru sayısını seçin',
  required = false
}) => {
  const fieldsetId = `question-count-selector-${Math.random().toString(36).substring(2, 9)}`;
  const hasError = Boolean(error);

  const handleChange = (selectedValue: QuestionCountOption) => {
    try {
      if (!disabled) {
        onChange(selectedValue);
      }
    } catch (error) {
      console.error('Error changing question count:', error);
      // Gracefully handle the error without breaking the UI
    }
  };

  const handleKeyDown = (event: React.KeyboardEvent, selectedValue: QuestionCountOption) => {
    try {
